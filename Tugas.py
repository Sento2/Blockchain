...
Versi 3: menutup 5 kesenjangan terhadap tabel & diagram di laporan yang
ditemukan saat review:
  1. index block auto-increment (Tabel 2.1), bukan diisi manual oleh caller
  2. timestamp block int64 Unix time (Tabel 2.1), dibulatkan dari time.time()
  3. Mekanisme Pencabutan (Revocation) whitelist (Bagian 3.2) ditambahkan
  4. Mempool eksplisit sebagai struktur data tersendiri (Diagram Arsitektur,
     komponen "Memory Pool - Antrean Transaksi"), bukan langsung dari list tx
  5. IBFT dipecah jadi 3 fase eksplisit: Pre-Prepare, Prepare, Commit
     (Bagian 4.2 & langkah 4 Diagram Arsitektur), bukan satu loop tanda tangan
"""
...
def register_operator(operator_id, public_key):
    """Registrasi & Otorisasi (Bagian 3.2): public key baru didaftarkan ke
    whitelist yang dikelola bersama oleh keempat Validator."""
...
def revoke_operator(operator_id):
    """Pencabutan (Bagian 3.2): bila node/akun terbukti disalahgunakan,
    public key-nya dicabut dari whitelist atas kesepakatan Validator, sehingga
    transaksi baru dari kunci tersebut otomatis ditolak jaringan berikutnya."""
    if operator_id in permission_list:
        del permission_list[operator_id]
        return True
    return False
...
# ============================================================
# 4b. MEMPOOL - "Memory Pool / Antrean Transaksi" pada Diagram Arsitektur,
#     komponen tersendiri antara Participant Node dan Proposer (langkah 2->3)
# ============================================================
mempool = []
...
def proposer_take_from_mempool(n=None):
    """Salah satu validator yang mendapat giliran sebagai Proposer mengambil
    transaksi dari Mempool untuk dibungkus jadi kandidat block (langkah 3
    Diagram Arsitektur). Transaksi yang diambil dikeluarkan dari antrean."""
    taken = mempool[:n] if n is not None else mempool[:]
    del mempool[: len(taken)]
    return taken
...
# ============================================================
_block_index_counter = [0]  # index: "Auto-increment saat block dibuat" (Tabel 2.1)


class Block:
    def __init__(self, transactions):
        _block_index_counter[0] += 1
        self.index = _block_index_counter[0]  # auto-increment, bukan diisi caller
        self.timestamp = int(time.time())  # int64 (Unix time), sesuai Tabel 2.1
...
# ============================================================
# 7. KONSENSUS IBFT - 3 fase eksplisit (Bagian 4.2 & langkah 4 Diagram)
# ============================================================
def ibft_pre_prepare(block):
    """Fase Pre-Prepare: Proposer menghitung header hash dari kandidat block
    dan menyiarkannya ke seluruh validator untuk diperiksa."""
    return block.compute_header_hash()


def ibft_prepare(offline_or_byzantine):
    """Fase Prepare: setiap validator yang online memeriksa kandidat block dan
    menyatakan siap ("prepared") untuk header hash yang sama. Validator yang
    offline/curang tidak ikut prepared, sehingga tidak lanjut ke fase Commit."""
    return [v_name for v_name in validators if v_name not in offline_or_byzantine]


def ibft_commit(block, header_hash_hex, prepared_validators):
    """Fase Commit: setiap validator yang prepared menandatangani header hash
    dengan private key-nya; signature disimpan ke validator_signatures[]."""
    header_hash_bytes = bytes.fromhex(header_hash_hex)
    approvals = 0
    for v_name in prepared_validators:
        keys = validators[v_name]
...
def run_ibft_consensus(block, offline_or_byzantine=None, threshold=2 / 3):
    """Menjalankan ketiga fase IBFT berurutan. Block sah & final hanya jika
    validator_signatures mencapai ambang >=66% (Bagian 4.3)."""
    offline_or_byzantine = offline_or_byzantine or []

    header_hash_hex = ibft_pre_prepare(block)                       # Fase 1
    prepared_validators = ibft_prepare(offline_or_byzantine)        # Fase 2
    approvals = ibft_commit(block, header_hash_hex, prepared_validators)  # Fase 3
...
if __name__ == "__main__":
    ...
    print(f"  Mempool sebelum Proposer mengambil transaksi: {mempool_size()} tx")
    block_A = Block(transactions=proposer_take_from_mempool())  # index auto-increment
    ...
    block_B = Block(transactions=proposer_take_from_mempool())
    ...
    block_C = Block(transactions=proposer_take_from_mempool())
    ...
    block_D = Block(transactions=proposer_take_from_mempool())
    ...
    section("SKENARIO UJI 4 - Pencabutan (Revocation) Operator dari Whitelist")
    print(f"  TELLER-01 terdaftar di Permission List? -> {is_registered('TELLER-01')}")
    revoke_operator("TELLER-01")
    print("  Validator sepakat mencabut TELLER-01 dari whitelist (mis. terbukti disalahgunakan)")
    print(f"  TELLER-01 terdaftar di Permission List? -> {is_registered('TELLER-01')}")

    tx7 = make_deposit("AGT-005", 250000, "TELLER-01", teller_private)
    ok, msg = participant_node_verify(tx7, teller_public)
    status_line(ok, f"Setoran {rp(tx7.amount)} - {tx7.member_id} (dari TELLER-01 pasca-pencabutan)", msg)
